...
class Szachownica{
	...
	void print(bool show){//czytanie planszy
		if(!show) return;
		system("cls");
		cout<<"  ";
		for(int i=1;i<=8;i++){
	...
	void move(int a, int b, int c, int d){//zrobienie ruchu
		if(p[c][d]==1||p[c][d]==7) zbicie_pkt=1;
		if(p[c][d]==2||p[c][d]==8) zbicie_pkt=5;
		if(p[c][d]==3||p[c][d]==9) zbicie_pkt=3;
		if(p[c][d]==4||p[c][d]==10) zbicie_pkt=3;
		if(p[c][d]==5||p[c][d]==11) zbicie_pkt=9;
		if(p[c][d]==6||p[c][d]==12) zbicie_pkt=100;
		p[c][d]=p[a][b];
		p[a][b]=0;
		if(p[c][d]==6){
			kb1=c, kb2=d;
	...
	bool check_move(int a, int b, int c, int d, int tr){//sprawdzenie legalności ruchu nie patrząc na zasady typu szach
		...
		if(p[a][b]==1||p[a][b]==7){//pionek
			...
			if(pk==a&&c==a-2*dr&&b==d&&!p[a-1*dr][b]&&!p[a-2*dr][b]) return true;
	...
	void fake_move(int a, int b, int c, int d){//ruch niepernamentny
		p[c][d]=p[a][b];
		p[a][b]=0;
		if(p[c][d]==6){
			kb1=c, kb2=d;
	...
	void zamiana_pionka(int a, int b, bool show){
		int nw=4;
		if(show){
	...
	int ruch_gracza(int a, int b, int c, int d, bool show){
		zbicie_pkt=0;

		if(!gd(a, b, c, d)){
			print(show);
			if(show) cout<<"Pole poza plansza\n";
			return 0;
		}
		if(!p[a][b]||kogo_bierek(a, b)!=tura){
			print(show);
			if(show) cout<<"Tu nie ma twojego pionka\n";
			return 0;
		}
		if(roszada(a, b, c, d)){//roszada-dobry ruch
			...
			if(spr(a, b, c, d)){
				print(show);
				if(show) cout<<"Nie mozesz zrobic takiego ruchu bo jestes/bedziesz pod szachem\n";
				return 0;
			}
			...
			move(a, b, c, d);
			print(show);
			if(c==1&&p[c][d]==1) zamiana_pionka(c, d, show);
			if(c==8&&p[c][d]==7) zamiana_pionka(c, d, show);
			print(show);
			tura=!tura;
			if(mat()) return -1;
			return 1;
			...
			print(show);
			if(show) cout<<"nie mozesz zrobic takiego ruchu\n";
			return 0;
		}
	}
};
